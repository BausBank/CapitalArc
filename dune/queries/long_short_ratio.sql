...
-- ----------------------------------------------------------------
-- Inferred long/short ratio from spot-DEX aggressor flow:
--   ratio = sum(buy_usd) / sum(sell_usd)
-- "long_account_pct" / "short_account_pct" approximate the
-- fraction of unique buyer / seller wallets - the closest thing
-- to a "% of accounts net-long" we can compute from the spot tape.
--
-- Parameters
-- ----------
--   {{chain}}              text
...
SELECT
    ...
    COALESCE(
        CAST(a.long_traders AS double)
        / NULLIF(a.long_traders + a.short_traders, 0),
        0.5
    ) AS long_account_pct,
    COALESCE(
        CAST(a.short_traders AS double)
        / NULLIF(a.long_traders + a.short_traders, 0),
        0.5
