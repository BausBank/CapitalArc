...
-- represented as the cumulative signed aggressor flow scaled into a
-- notional funding cost using the same 0.0005 / 8h coefficient
-- used in funding_rates.sql.
--
--   net_signed_flow = sum(buy_usd) - sum(sell_usd)
--   notional        = |net_signed_flow| * 0.0005 * (hours / 8)
--
-- Positive net flow means the market spent more USD buying the
...
SELECT
    ...
    COALESCE(GREATEST(a.gross_buy_usd - a.gross_sell_usd, 0)
             * 0.0005 * ({{lookback_hours}} / 8.0), 0) AS longs_paid_usd,
    COALESCE(GREATEST(a.gross_sell_usd - a.gross_buy_usd, 0)
             * 0.0005 * ({{lookback_hours}} / 8.0), 0) AS shorts_paid_usd,
    COALESCE((a.gross_buy_usd - a.gross_sell_usd)
             * 0.0005 * ({{lookback_hours}} / 8.0), 0) AS net_flow_usd,
